    ...
    <footer className="mt-12 border-t border-white/10 bg-black/30 px-4 py-8 md:px-8">
      <div className="mx-auto grid max-w-7xl gap-7 lg:grid-cols-[1fr_1fr_1fr]">
        <div className="flex items-center gap-4">
          <img src={basketMotionAiLogo} alt={`${BRAND_NAME} logo`} className="h-14 w-14 rounded-xl object-cover ring-1 ring-white/10" />
          <div>
            <div className="text-sm font-black uppercase tracking-widest text-white">{BRAND_NAME}</div>
            <div className="text-xs text-white/40">{BRAND_TAGLINE}</div>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <a href="https://www.instagram.com/BasketMotionAI" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm font-bold text-white/80 hover:text-brand-orange">
              <Instagram size={18} /> @BasketMotionAI
            </a>
          </div>
          <div>
